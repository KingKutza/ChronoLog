...
void main() {
  ...
  test('no staple, no projection: the refusal is honest and worded, whatever the bases', () {
    ...
    for (var draw = 0; draw < 10; draw++) {
      ...
      expect(
        projection.framesProject(frameA, frameB),
        isFalse,
      ...
      expect(
        answer.refused.single.message,
        contains('Staple a point'),
  ...
  test('pick up any, pick up all: one identification relates both ways, chains carry', () {
    ...
    for (var draw = 0; draw < 10; draw++) {
      ...
      expect(
        projection.framesProject(frameB, frameA),
        isTrue,
      ...
      expect(
        FrameProjection(document.toJson()).framesProject('frame:gamma', frameA),
        isTrue,
  ...
  test(
    'exact at every stapled point, monotone when laid sequentially, refinement moves nothing',
    () {
      ...
      for (var draw = 0; draw < 10; draw++) {
        ...
        for (final entry in entries) {
          expect(
            staples.frameEndDays(entry.from),
            isNotNull,
        ...
        expect(
          staples.describeCorrespondence(frameA, frameB).monotonic,
          isTrue,
          reason:
              'seed $runSeed draw $draw: laid sequentially along both frames, and the '
        ...
        final refined = Staples(sequentialStaples(marked.document, random, lawA, lawB).document);
        final after = [
        ...
        for (final pair in before) {
          expect(
            ...
            contains(pair),
            reason:
                'seed $runSeed draw $draw: a new staple refines the function; '
        ...
        Relation(
          ...
          extra: {
            ...
            'ends': [
              ...
              FrameEnd(
                frameA,
                position: Position.coordinate(coordinateAt(lawA, 5 + random.nextInt(500))),
              ).toJson(),
            ],
          },
        ),
  ...
  test("e'=A'=B' and e''=A''=B'' as the pair encoding: one point per sheet, no contest", () {
    ...
    for (var draw = 0; draw < 10; draw++) {
      ...
      document = document.put(
        ...
        Relation(
          ...
          extra: {
            ...
            'ends': [
              ObjectEnd('event:e', point: 'start').toJson(),
              FrameEnd(frameA, position: Position.coordinate(placedAt)).toJson(),
            ],
          },
        ),
      );
      ...
      for (final (point, frame, law, total) in [
        ...
        // (ruled 2026-09-01), and a whole is not an instant an anchor can name.
        ('start', frameB, lawB, 3 + random.nextInt(400)),
      ...
      expect(
        [...extent.overdetermined, ...extent.unresolved],
        isEmpty,
        reason:
            'seed $runSeed draw $draw: claims on different sheets are one '
      ...
      for (final (point, frame, at) in authored) {
        expect(
          ...
          staples.frameEndDays(StapleEnd.frame(frame, position: Position.coordinate(at))),
          reason: 'seed $runSeed draw $draw: $point of e, as written on $frame',
        );
      }
      for (final point in ['start', 'end']) {
        expect(
          extent.positions[point]!.keys,
          unorderedEquals([staples.spaceOfFrame(frameA), staples.spaceOfFrame(frameB)]),
          reason:
              'seed $runSeed draw $draw: two sheets, both spelled, neither '
  ...
  test("e'=A'=B' as ONE staple: the frames correspond through e, and no claim is a contest", () {
    ...
    for (var draw = 0; draw < 6; draw++) {
      ...
      document = putStaple(
        ...
        kind: 'anchor',
        ends: [
          StapleEnd.object('event:e', point: 'start'),
      ...
      expect(
        FrameProjection(document.toJson()).framesProject(frameA, frameB),
        isTrue,
        reason:
            'seed $runSeed draw $draw: n points on objects or frames are one point, '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'seed $runSeed draw $draw: one point on three sheets is an '
  ...
  test('the piecewise function derived from the staple set is evaluable from any angle', () {
    ...
    for (var draw = 0; draw < 8; draw++) {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'seed $runSeed draw $draw: nothing relates the two bases, so '
      ...
      for (final mark in ordered) {
        ...
        expect(
          landed.at,
          {for (final pair in marks[mark]!) pair.to}.toList()
            ..sort((left, right) => left.compareTo(right)),
          reason:
              'seed $runSeed draw $draw: the authored instants, all of '
              'them, and nothing invented between them',
        );
        for (final pair in marks[mark]!) {
          expect(
            correspondence.landing(frameB, frameA, pair.to).at,
            contains(mark),
        ...
        for (var step = 1; step < steps; step++) {
          ...
          final expected = <Rational>{
            for (final lower in marks[low]!)
              for (final upper in marks[high]!) lower.to + (upper.to - lower.to) * ratio,
          }.toList()..sort((left, right) => left.compareTo(right));
          expect(
            landed.at,
            expected,
            reason:
                'seed $runSeed draw $draw: the stretch IS the authored '
                'ratio, never averaged and never interpolated across a gap',
          );
          if (marks[low]!.length == 1 && marks[high]!.length == 1) {
            expect(
              correspondence.landing(frameB, frameA, landed.at.single).at,
              contains(on),
              reason:
                  'seed $runSeed draw $draw: one function, read from the '
      ...
      expect(
        refined.anchorsBetween(frameA, frameB).length,
        greaterThan(pairs.length),
        reason: 'seed $runSeed draw $draw',
      );
      for (final pair in pairs) {
        expect(
          refined.landing(frameA, frameB, pair.from).at,
          contains(pair.to),
          reason:
              'seed $runSeed draw $draw: the second stapler refines the '
