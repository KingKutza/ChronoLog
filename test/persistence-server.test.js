...
test("two local clients writing to the same workspace keep both revisions on conflict", async () => {
  const root = await mkdtemp(join(tmpdir(), "chronolog-conflict-"));
  let running;
  try {
    running = await startServer(root);
    const first = '{"schema":"chronolog/1","client":"A"}\n';
    const writeA = await fetch(`${running.url}/api/document`, { method: "PUT", body: first });
    ...
    const writeB = await fetch(`${running.url}/api/document`, {
      method: "PUT", headers: { "if-match": revisionA }, body: second
    });
    assert.equal(writeB.status, 204);
    const staleA = await fetch(`${running.url}/api/document`, {
      method: "PUT", headers: { "if-match": revisionA }, body: '{"schema":"chronolog/1","client":"A-conflict"}\n'
    });
